...
class Section_about extends CI_Controller {
		...
		if (is_post()) {
			...
			if ($_FILES['userfile']['size'] > 0 ){
				...
				if (!$upload[0]){
					...
					redirect(base_url("manage/section_about/edit/$id"));
				}else{
					$item['image'] = $upload[1]['file_name'];
				}	
			}
			$this->db->where('id', $id);
			$this->db->update('section_about', $item); 
			$this->session->set_flashdata('message','Data Saved Successfully');
			redirect(base_url("manage/section_about/edit/$id"));
		}
		$data['items'] 		= $this->db->get_where('section_about',array('id'=>$id))->row();
		// debug_array($data,true);
		$data['local_view'] = 'v_section_about';
